...
export default function ClothingProduct({ cart, setCart }) {
  ...
  const similarProducts = product
    ? Clothing.filter((item) => item.id !== product.id).slice(0, 4)
    : [];
        ...
        <div className="mt-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {similarProducts.map((item) => (
              <Link to={`/clothing/${item.id}`} key={item.id} className="group">
